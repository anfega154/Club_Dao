...
public class Socio {
    ...
    //****************************************Base de Datos*******************************************************
    public String MostrarfacturaSocioBD(int cedula) throws SQLException {
        ClubDao clubd = new ClubDao();
        
        ArrayList<Factura> facturas = clubd.TraerFacturasSocio(cedula);
        String mFactura = "";
        
        for (int i = 0; i < facturas.size(); i++) {
            mFactura += facturas.get(i).toString();
        }
        return mFactura;
        
    }
    
    public Factura BuscarFacturaSocioBD(int cedula, int posicion) throws SQLException {
        ClubDao clubd = new ClubDao();
        ...
        if (socio == null) {
            ...
            if (this.MostrarfacturaSocioBD(cedula).equals("")) {
                JOptionPane.showMessageDialog(null, "el socio no tiene Facturas");
            } else {
                for (int i = 0; i < facturas.size(); i++) {
                    if (facturas.get(i).getId() == posicion) {
    ...
    public void pagarFacturaSocioBD(int cedula, int posicion) throws SQLException {
        ClubDao clubd = new ClubDao();
        Socio socio = clubd.BuscarSocio1(cedula);
        String facturas = MostrarfacturaSocioBD(cedula);
        Factura fac = BuscarFacturaSocioBD(cedula, posicion);
        if (facturas.equals("")) {
            JOptionPane.showMessageDialog(null, "el socio no tiene facturas pendientes de pago");
        } else {
            if (fac != null) {
                if (fac.getValor() <= socio.getFondos()) {
                    double fondosnew = socio.getFondos() - fac.getValor();
                    this.imprimirFacturaSocioBD(cedula, posicion);
